...
- `actions`: action list, for example:
    ...
    | `run_agent` | Asynchronously submit to `claw_core`; Agent response is published back via `agent_response` event |
...
`basic_demo` ships a starter `router_rules.json` for basic IM flows. See <LinkToSource relativePath="application/basic_demo/fatfs_image/router_rules/router_rules.json" />.
...
<Aside type="tip" title="Editing router_rules.json by hand">
    In `basic_demo`, the file lives at `/fatfs/router_rules/router_rules.json`.
...
For example, `basic_demo` includes rule `agent_stage_im_notify` in <LinkToSource relativePath="application/basic_demo/fatfs_image/router_rules/router_rules.json" />. When Verbose is enabled and this rule matches, it sends `{{event.text}}` from `agent_stage` back to the source IM conversation.
