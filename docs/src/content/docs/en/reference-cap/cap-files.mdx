...
description: Filesystem reference capability—full CRUD over a managed FATFS tree with path-safety rules.
---
...
<Aside type="caution">
  `write_file` and `edit_file` let the LLM change any managed file—including skills and Lua. Treat this capability carefully for multi-tenant or untrusted models, or narrow writable paths with access control.
</Aside>
