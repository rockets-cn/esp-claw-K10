...
<Aside type="note" title="Board-gated modules">
    In `basic_demo`, `audio`, `camera`, and `lcd_touch` are compiled in only when the board Kconfig says the hardware exists (`CONFIG_ESP_BOARD_DEV_AUDIO_CODEC_SUPPORT`, `CONFIG_ESP_BOARD_DEV_CAMERA_SUPPORT`, `CONFIG_ESP_BOARD_DEV_LCD_TOUCH_I2C_SUPPORT`); `basic_demo_lua_modules_register()` registers them only in that case.  
...
The `data` parameter can be a Lua `string` or `lightuserdata` (e.g., `camera.frame:ptr()`). The number of bytes must be `src_width * src_height * 2`. For a typical usage, see the camera preview example at <LinkToSource relativePath="application/basic_demo/fatfs_image/scripts/camera_preview_demo.lua" />.
