...
description: 文件系统代表：受管 FATFS 目录的完整 CRUD 能力与路径安全机制。
---
...
`cap_files` 是**文件系统类 Capability 的代表实现**，向 LLM 和自动化规则暴露对受管 FATFS 目录的完整读写能力。它展示了如何在嵌入式环境中安全地将文件系统访问抽象为工具接口。
...
<Aside type="caution">
  `cap_files` 的 `write_file` 和 `edit_file` 允许 LLM 修改设备上的任何受管文件，包括 Skill 文档和 Lua 脚本。在多用户或不可信 LLM 场景下，应谨慎开放此 Capability，或通过访问控制限制可写目录。
</Aside>
