...
<Aside type="note" title="板级条件模块">
    `audio`、`camera`、`lcd_touch` 三个模块在 `basic_demo` 中受板级 Kconfig 条件编译控制：仅当开发板硬件支持对应外设（`CONFIG_ESP_BOARD_DEV_AUDIO_CODEC_SUPPORT`、`CONFIG_ESP_BOARD_DEV_CAMERA_SUPPORT`、`CONFIG_ESP_BOARD_DEV_LCD_TOUCH_I2C_SUPPORT`）时，`basic_demo_lua_modules_register()` 才会注册这些模块。  
...
`data` 参数可以是 Lua `string` 或 `lightuserdata`（如 `camera.frame:ptr()`），字节数需为 `src_width * src_height * 2`。典型用法见摄像头预览示例 <LinkToSource relativePath="application/basic_demo/fatfs_image/scripts/camera_preview_demo.lua" />。
